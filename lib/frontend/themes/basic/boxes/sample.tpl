<div id="cart-box-quote" class="cart-box{if $settings[0].show_products == 'dropdown'} hover-box{/if}">
  <a class="cart-box-link" href="{tep_href_link('sample-cart/index')}">
    <span class="no-text">
      <strong class="strong">{$smarty.const.TEXT_HEADING_SAMPLE_CART}</strong>
  ...
  <div class="cart-content">
    ...
    {/foreach}
    <div class="buttons">
      <div class="left-buttons"><a href="{tep_href_link('sample-cart')}" class="btn">{$smarty.const.TEXT_HEADING_SHOPPING_CART}</a></div>
    </div>
  </div>
  {*/if*}
